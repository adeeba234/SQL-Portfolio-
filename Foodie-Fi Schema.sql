...
as count_customer,Round(count(customer_id)/(select count(customer_id) 
from subscriptions),3)*100 as Percentage 
from subscriptions
 where plan_id=4 and customer_id
 in(select customer_id from subscriptions where plan_id=0);
...
from next_plane_cte
where next_plane is not null and plan_id
group by next_plane
order by next_plane;
...
select plan_name,count(customer_id)
 as count from subscriptions 
 as sp join plans as p 
